...
class ApiLoader::bfl_Api {
 public:
  ...
  bfl_Api() {
    auto module = Globals().GetModuleLoader().GetModule("bifrost_loader.dll");
...
ApiLoader::ApiLoader() { m_api = std::make_unique<bfl_Api>(); }

ApiLoader::~ApiLoader() = default;

void ApiLoader::Register(const Plugin& plugin) const {
  bfl_Plugin p;
...
void ApiLoader::Reset() const { BIFROST_CHECK_BFL_CALL(m_api->bfl_Reset(), "Failed to reset plugins"); }

const char* ApiLoader::GetVersion() const { return m_api->bfl_GetVersion(); }
