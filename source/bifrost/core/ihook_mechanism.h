...
class IHookMechanism {
 public:
  ...
  virtual void SetUp(HookContext* ctx) = 0;

  /// Tear-down and cleanup
  virtual void TearDown(HookContext* ctx) = 0;

  /// Set the hook from `target` to `detour` and return the originally registered function in `original`
  virtual void SetHook(HookContext* ctx, const HookTarget& target, void* detour, void** original) = 0;

  /// Remove any hook that has been set to `target`
  virtual void RemoveHook(HookContext* ctx, const HookTarget& target) = 0;
