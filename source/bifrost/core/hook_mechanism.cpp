...
void MinHook::SetHook(Context* ctx, HookDebugger* debugger, void* target, void* detour, void** original) {
  BIFROST_CHECK_MH(MH_CreateHook(target, detour, original),
                   StringFormat("to create hook from %s to %s", debugger->SymbolFromAdress(ctx, target), debugger->SymbolFromAdress(ctx, detour)));
  BIFROST_CHECK_MH(MH_EnableHook(target), StringFormat("to enable hook from %s", debugger->SymbolFromAdress(ctx, target)));
  debugger->AddTrampoline(*original, target);
}

void MinHook::RemoveHook(Context* ctx, HookDebugger* debugger, void* target) {
  BIFROST_CHECK_MH(MH_DisableHook(target), StringFormat("to disable hook from %s", debugger->SymbolFromAdress(ctx, target)));
  BIFROST_CHECK_MH(MH_RemoveHook(target), StringFormat("to remmove hook from %s", debugger->SymbolFromAdress(ctx, target)));
  debugger->RemoveTrampoline(target);
}
...
void VTableHook::SetHook(Context* ctx, HookDebugger* debugger, void* target, void* detour, void** original) {}

void VTableHook::RemoveHook(Context* ctx, HookDebugger* debugger, void* target) {}
