...
void EnumerateThreads(Context* ctx, FilterFunc&& filterFunc) {
  HANDLE snapshot;
  BIFROST_ASSERT_WIN_CALL_CTX(ctx, (snapshot = ::CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0)) != NULL);
...
Process::Process(Context* ctx, std::wstring_view name) : Object(ctx) {
  ...
  if (snapshot != INVALID_HANDLE_VALUE) BIFROST_CHECK_WIN_CALL(::CloseHandle(snapshot) != FALSE);
...
void Process::Inject(InjectArguments args) {
    try {
      ...
      switch (stage) {
        ...
        case bifrost::E_Done:
          if (errorCode != 0) {
            error(StringFormat(L"failed, init procedure '%s' returned", StringToWString(args.InitProcName).c_str()).c_str(),
...
void KillProcess(Context* ctx, std::wstring_view name, bool failOnError) {
    ...
    if (snapshot != INVALID_HANDLE_VALUE) BIFROST_CHECK_WIN_CALL_CTX(ctx, ::CloseHandle(snapshot) != FALSE);
}
